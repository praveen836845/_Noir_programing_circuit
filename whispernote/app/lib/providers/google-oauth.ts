...
export const GoogleOAuthProvider: AnonGroupProvider = {
  ...
  generateProof: async (ephemeralKey: EphemeralKey) => {
    ...
    if (!domain) {
      throw new Error(
        "You can only use this app with a Google Workspace account from your organization. " +
    ...
    if (domain === 'gmail.com') {
      throw new Error(
        ...
        "Please use your organization's Google Workspace account instead. " +
        "Contact your IT administrator if you need help setting up your work account."
      );
    }
